//! Tests for runtime config resolution (platform-independent logic only).

use smsgate::creds::RuntimeConfig;
...
fn runtime_config_keeps_loaded_values_when_compiled_config_is_not_applied() {
    ...
    let resolved = RuntimeConfig::resolve_compiled_config(loaded.clone(), false);
    ...
    assert_eq!(
        ...
        loaded.max_failures_before_reboot
    );
    assert_eq!(resolved.poll_interval_ms, loaded.poll_interval_ms);
}
...
fn runtime_config_uses_compiled_defaults_when_compiled_config_is_applied() {
    ...
    let resolved = RuntimeConfig::resolve_compiled_config(loaded, true);
    let compiled = RuntimeConfig::default();
    ...
    assert_eq!(
        resolved.max_failures_before_reboot,
        compiled.max_failures_before_reboot
    );
